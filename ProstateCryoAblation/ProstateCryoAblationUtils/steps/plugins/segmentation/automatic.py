...
from base import ProstateCryoAblationSegmentationPluginBase
from ...base import ProstateCryoAblationLogicBase


class ProstateCryoAblationAutomaticSegmentationLogic(ProstateCryoAblationLogicBase):
  ...
  def __init__(self):
    super(ProstateCryoAblationAutomaticSegmentationLogic, self).__init__()
    self.tempDir = os.path.join("/tmp", "ProstateCryoAblation")
    self.inputVolume = None
    if not os.path.exists(self.tempDir):
      self.createDirectory(self.tempDir)

  def cleanup(self):
    super(ProstateCryoAblationAutomaticSegmentationLogic, self).cleanup()
    try:
...
class ProstateCryoAblationAutomaticSegmentationPlugin(ProstateCryoAblationSegmentationPluginBase):

  NAME = "AutomaticSegmentation"
  LogicClass = ProstateCryoAblationAutomaticSegmentationLogic

  def __init__(self):
    super(ProstateCryoAblationAutomaticSegmentationPlugin, self).__init__()
  ...
  def cleanup(self):
    super(ProstateCryoAblationAutomaticSegmentationPlugin, self).cleanup()
    self.logic.cleanup()

  def setup(self):
    super(ProstateCryoAblationAutomaticSegmentationPlugin, self).setup()
  ...
  def onSegmentationFinished(self, caller, event, labelNode):
    ...
    super(ProstateCryoAblationAutomaticSegmentationPlugin, self).onSegmentationFinished(caller, event, labelNode)
